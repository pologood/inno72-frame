...
import static org.apache.flume.sink.kafka.KafkaSinkConstants.ZOOKEEPER_CONNECT;
...
//		String eventKey = null;
		...
		try {
			...
			for (; processedEvents < batchSize; processedEvents += 1) {
				...
				String bodyJson = new String(eventBody);
				...
				try {
					if ( bodyObject != null ) {
						String logType = java.util.Optional.ofNullable(bodyObject.get("logType")).map(Object::toString).orElse("");
						logger.info(" kafakaSink 解析后的 logType ===============>  {}", logType);
						logger.info(" kafakaSink 解析后的 eventBody ===============>  {}", bodyJson);
						TopicEnum findByType = TopicEnum.findByType(logType);
						if (findByType != null) {
							kafkaFutures.add(producer.send(new ProducerRecord<String, byte[]>(findByType.getTopic(), null, eventBody), new SinkCallback(startTime)));
						}
					}
	...
	private void translateOldProps(Context ctx) {
		...
		if (!(ctx.containsKey(BATCH_SIZE))) {
			...
			if ( oldBatchSize != null  && !oldBatchSize.isEmpty())  {
				ctx.put(BATCH_SIZE, defaultBatchSize);
				logger.warn("{} is deprecated. Please use the parameter {}", defaultBatchSize, BATCH_SIZE);
			}
		}
		
		//zkConnect Size
		if (!(ctx.containsKey(ZOOKEEPER_CONNECT))) {
			String zkConnect = ctx.getString(ZOOKEEPER_CONNECT);
			if ( zkConnect != null  && !zkConnect.isEmpty())  {
				ctx.put(ZOOKEEPER_CONNECT, zkConnect);
				logger.warn("{} is deprecated. Please use the parameter {}", zkConnect, ZOOKEEPER_CONNECT);
			}
		}
